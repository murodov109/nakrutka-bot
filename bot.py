...
BOT_TOKEN = "8382109071:AAGsX1zJY7cqvVFekJTXDbYHP8nfRT8tYvk"
ADMIN_IDS = [123456789]
...
if not os.path.exists("data.json"):
    with open("data.json", "w") as f:
        json.dump({"users": {}, "tasks": {}, "orders": {}, "channels": [], "admins": ADMIN_IDS}, f)
...
def admin_panel(message):
    if message.from_user.id not in ADMIN_IDS:
        return
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("📊 Statistika", "💳 Karta raqamini o‘zgartirish")
    markup.add("🧩 Vazifa qo‘shish", "🔗 Majburiy kanallar")
